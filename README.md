...
### Challenges
Due to compute-intensive requirements of NetShare, reproducing their results on [CAIDA](https://www.caida.org/catalog/datasets/passive_dataset/) (PCAP) and [UGR16](https://nesg.ugr.es/nesg-ugr16/) (NetFlow) was tough with our setup. So we resorted to using a P2P [BotNet](https://github.com/dmbb/FlowLens) header trace and regenerated a synthetic malicious trace using NetShare. Owing to the relative simlicity of this dataset, it could be generated on a smaller testbed using NetShare. But we wanted to evaluate NetShare on various other traces as well, preferrably using GANs trained on a bigger cluster. Therefore, we contacted the authors and requested synthetic CAIDA and UGR16 traces generated from NetShare on their 200 CPU cluster, which they provided to us very kindly.
